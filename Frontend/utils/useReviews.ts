...
export const useReviews = (toiletId: number) => {
  ...
  const fetchReviews = async () => {
    try {
      ...
      reviews.value = fetched.map((review: { id: number; user_id: number | null }) => {
  ...
  const toggleReaction = async (reviewId: number) => {
    ...
    try {
      if (existing) {
        const res = await fetch(`${BASE_URL}/reactions/${existing.id}`, { method: 'DELETE' })
        if (!res.ok) throw new Error('取消失敗')
        reactions.value[reviewId] = existingList.filter(r => r.id !== existing.id)
      } else {
        const res = await fetch(`${BASE_URL}/reactions/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ review_id: reviewId, user_id: user.id, is_liked: true })
        })
